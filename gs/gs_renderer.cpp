...
bool GSRenderer::init(Vulkan::Device *device_, const GSOptions &options)
{
	...
	// Used by de-interlacer.
	Vulkan::SamplerCreateInfo samp_info = {};
	...
	float_border_sampler = device->create_sampler(samp_info);
	samp_info.border_color = VK_BORDER_COLOR_INT_TRANSPARENT_BLACK;
	int_border_sampler = device->create_sampler(samp_info);
...
ScanoutResult GSRenderer::vsync(const PrivRegisterState &priv, const VSyncInfo &info)
{
	...
	if (is_interlaced || force_deinterlace)
	{
		...
		vsync_last_luma_pyramids[1] = std::move(vsync_last_luma_pyramids[0]);
		vsync_last_luma_pyramids[0] = compute_luminance_hierarchy(cmd, merged->get_view());
		...
		if (!vsync_last_luma_pyramids[1])
			vsync_last_luma_pyramids[1] = vsync_last_luma_pyramids[0];

		auto motion = compute_optical_flow(
				cmd, *vsync_last_luma_pyramids[0], *vsync_last_luma_pyramids[1]);

		// Crude de-interlace. Get something working for now.
		merged = motion_deinterlace(cmd, motion->get_view(), info);
	}
	else
	{
...
Vulkan::ImageHandle GSRenderer::motion_deinterlace(Vulkan::CommandBuffer &cmd, const Vulkan::ImageView &motion, const VSyncInfo &vsync)
{
	...
	push.inv_field_resolution.x = 1.0f / float(vsync_last_fields[0]->get_width());
	...
	for (int i = 0; i < 4; i++)
		cmd.set_texture(0, i, vsync_last_fields[i]->get_view(), Vulkan::StockSampler::LinearClamp);
	cmd.set_texture(0, 4, motion);
	cmd.set_texture(0, 5, vsync_last_luma_pyramids[1]->get_view(), Vulkan::StockSampler::NearestClamp);
...
Vulkan::ImageHandle GSRenderer::compute_luminance_hierarchy(Vulkan::CommandBuffer &cmd, const Vulkan::ImageView &view)
{
	uint32_t luma_width = (view.get_view_width() + 31u) & ~31u;
	uint32_t luma_height = (view.get_view_height() + 31u) & ~31u;

	auto info = Vulkan::ImageCreateInfo::immutable_2d_image(
			luma_width, luma_height, VK_FORMAT_R16_UNORM);
	...
	Vulkan::ImageViewHandle views[5];
	...
	cmd.begin_region("luminance-hierarchy");
	cmd.set_program(shaders.luminance_pyramid);
	for (unsigned level = 0; level < info.levels; level++)
		cmd.set_storage_texture(0, level, *views[level]);
	cmd.set_texture(0, 5, view, Vulkan::StockSampler::NearestClamp);
	struct Push
	{
		vec2 inv_resolution;
		int mips;
	} push = {};
	push.inv_resolution.x = 1.0f / float(view.get_view_width());
	push.inv_resolution.y = 1.0f / float(view.get_view_height());
	push.mips = 5;
	cmd.push_constants(&push, 0, sizeof(push));
...
Vulkan::ImageHandle GSRenderer::compute_optical_flow(Vulkan::CommandBuffer &cmd,
                                                     const Vulkan::Image &current,
                                                     const Vulkan::Image &prev)
{
	// Inspired heavily by FidelityFX Optical Flow in FSR3.
	// Hierarchical search algorithm in 8x8 blocks.
	// Some obvious changes from FSR3:
	// - Fewer levels (much lower resolution).
	// - Final pass upscales from 8x8 blocks to 4x4 blocks.
	// - QPEL motion refine is done per 4x4 block.
	uint32_t mv_width = current.get_width() / 8;
	uint32_t mv_height = current.get_height() / 8;

	cmd.begin_region("optical-flow");
	Vulkan::ImageHandle previous_level_upscale;
	...
	for (int level = 4; level >= 0; level--)
	{
		...
		Vulkan::ImageHandle search_img = previous_level_upscale ? previous_level_upscale : device->create_image(info);
		...
		cmd.begin_region("search");
		{
			...
			cmd.set_program(shaders.motion_search);
			cmd.set_specialization_constant_mask(1);
			cmd.push_constants(&push, 0, sizeof(push));
		...
		cmd.begin_region("filter");
		{
			...
			cmd.set_program(shaders.motion_filter);
			cmd.push_constants(&push, 0, sizeof(push));
	...
	auto info = Vulkan::ImageCreateInfo::immutable_2d_image(mv_width, mv_height, VK_FORMAT_R16G16_SINT);
	...
	{
		cmd.set_program(shaders.motion_search_qpel);
		cmd.set_storage_texture(0, 0, qpel_img->get_view());
		cmd.set_texture(0, 1, current.get_view());
		cmd.set_texture(0, 2, prev.get_view(), Vulkan::StockSampler::LinearClamp);
